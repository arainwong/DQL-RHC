...
class Args:
    ...
    eta: float = 1.0
    """normalize Q value in policy training, actor_loss = bc_loss + self.eta * q_loss"""
...
if __name__ == "__main__":
    ...
    for iteration, data_batch in enumerate(train_dataloader):
        ...
        for t in range(args.act_horizon):
            q1, q2 = critic.forward(obs_t, act_t)
            q1 = q1.squeeze(-1)
            q2 = q2.squeeze(-1)
            ...
            Q1_seq.append(q1)
            Q2_seq.append(q2)
        Q1_seq = torch.stack(Q1_seq, dim=-1)
        Q2_seq = torch.stack(Q2_seq, dim=-1)

        if horizon_comp_mode == 'uniform':
            Q1_seq_mean = Q1_seq.mean() # (1,)
            Q2_seq_mean = Q2_seq.mean() # (1,)
        elif horizon_comp_mode == 'exponential_decay':
        ...
        q_loss = - args.eta * (Q_seq_mean.mean())/(normalizer_Q.abs().mean().detach())
        ...
        if iteration % args.log_freq == 0:
            ...
            print(f"------------------------------------------------------")
            print(f"q_loss: {q_loss.item()}, Q mean = {Q_seq.mean().item()}/{normalizer_Q.abs().mean().item()}, Q: {Q_seq_mean[:4, :].flatten()}")
